...
`;
function ValidationInput({ validations, onChange, onBlur, ...props }) {
	const [inputError, setInputError] = (0, import_react.useState)(null);
	(0, import_react.useEffect)(() => {
		props.onChangeError?.(inputError);
	}, [inputError]);
	const handleOnChange = (event) => {
		const failedValidation = validations.find((validation) => event.target.value.length && validation.type === "limit" && !validation.validator(event.target.value));
		if (failedValidation) {
	...
	const handleOnBlur = (event) => {
		onBlur?.(event);
		const failedValidation = validations.find((validation) => typeof props.value === "string" && props.value.length && validation.type === "check" && !validation.validator(props.value));
		if (failedValidation) {
			setInputError(new Error(failedValidation.message));
			return;
		}
...
function validateNumberString(input) {
	return input.split("").every((char) => !isNaN(Number(char)));
}
...
function validateCVC(input) {
	return Array.from({ length: 1e3 }).map((_, index) => String(index).padStart(3, "0")).includes(input);
}
//#endregion
...
//#endregion
//#region src/hooks/useCardBrand.ts
...
function App() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(View, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(CardPreview, {
		cardBrand: useCardBrand(formState.cardNumberSegments),
